        ...
        <form action="{{url('admin/users/'.$users->id)}}" method="post">
            ...
            <div class="mb-3">
                ...
                <select class="form-select form-select-sm" name='division' aria-label=".form-select-sm example">
                    ...
                    <option value="{{ $division->id }}" <?=$users->division->id == $division->id ? 'selected' : ''?>>{{$division->name}}</option>
                    @endforeach
                  </select>
            </div>
            <div class="mb-3">
                ...
                <select class="form-select form-select-sm" name='position' aria-label=".form-select-sm example">
                    @foreach ($positions as $position)
                        <option value="{{ $position->id }}" <?=$users->position->id == $position->id ? 'selected' : ''?>>{{$position->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
